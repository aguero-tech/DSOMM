...
export class MatrixComponent implements OnInit {
  ...
  constructor(private yaml: ymlService, private router: Router) {
    ...
    this.filteredActivities = this.rowCtrlActivity.valueChanges.pipe(
      ...
      map((activity: string | null) =>
        activity ? this.filterActivity(activity) : this.autoCompleteActivityResults.slice()
      )
    );
  }
  ...
  ngOnInit(): void {
    ...
    this.yaml.getJson().subscribe(data => {
      ...
      for (let i = 0; i < this.allDimensionNames.length; i++) {
        const subdimensionsInCurrentDimension = Object.keys(this.YamlObject[this.allDimensionNames[i]]);
        for (let j = 0; j < subdimensionsInCurrentDimension.length; j++) {
          ...
          const activityInCurrentSubDimension: string[] = Object.keys(
            this.YamlObject[this.allDimensionNames[i]][subdimensionsInCurrentDimension[j]]
          );
          ...
          for (let a = 0; a < activityInCurrentSubDimension.length; a++) {
            ...
            const tagsInCurrentActivity: string[] =
              this.YamlObject[this.allDimensionNames[i]][subdimensionsInCurrentDimension[j]][currentActivityName].tags;
            ...
            try {
              const lvlOfActivity: number =
                this.YamlObject[this.allDimensionNames[i]][subdimensionsInCurrentDimension[j]][currentActivityName]['level'];
              (temp[this.lvlColumn[lvlOfActivity - 1] as keyof number] as unknown as string[]).push(currentActivityName);
            } catch {
  ...
  toggleSubDimensionSelection(subDimension: string): void {
    if (this.currentSubDimensions.includes(subDimension)) {
      ...
      this.currentSubDimensions = this.currentSubDimensions.filter(sd => sd !== subDimension);
  ...
  private filterSubDimension(value: string): string[] {
    return this.autoCompeteResults.filter(row =>
      row.toLowerCase().indexOf(value.toLowerCase()) === 0
    );
  }

  private filterActivity(value: string): string[] {
    return this.autoCompleteActivityResults.filter(activity =>
      activity.toLowerCase().indexOf(value.toLowerCase()) === 0
    );
  }
